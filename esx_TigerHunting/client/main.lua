...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] Bắt đầu báo trắng', ['x'] = -651.21, ['y'] = 1417.11, ['z'] = 304.35 },
	['Sell'] = { ['hint'] = '[E] Bán Thịt Báo', ['x'] = 6.9, ['y'] = -1816.6, ['z'] = 25.4 },
}
			...
			for index, value in pairs(Positions) do
				if value.hint ~= nil then
					...
					if OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] ~g~Dừng ~r~Săn Thú'
					elseif not OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] ~g~Bắt đầu ~r~Săn Thú'
					end
	...
	if OnGoingHuntSession then
		...
		Citizen.CreateThread(function()
			...
			for index, value in pairs(AnimalPositions) do
				...
				AddTextComponentString("<font face = 'Oswald'>Báo Trắng")
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Xẻ Thịt', 0.4)
...
BeginTextCommandSetBlipName("STRING")
AddTextComponentString("<font face = 'Oswald'>Săn Báo Trắng")
...
SetBlipAsShortRange(blip, true)
BeginTextCommandSetBlipName("STRING")
AddTextComponentString("<font face = 'Oswald'>Bán Thịt Báo")
EndTextCommandSetBlipName(blip)
